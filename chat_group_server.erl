...
handle_call({send_group_msg, {Message, Sender}}, _From, State) ->
    Members = State#state.members,
    Group = State#state.group_name,
    lists:foreach(fun(HandlerPid) -> gen_statem:cast(HandlerPid, {group_msg, {Group, Message, Sender}}) end, Members),
    {reply, {ok, "Group Message Sent"}, State}.
